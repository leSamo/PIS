...
public class Users {
    ...
    public List<UserDTO> getUsers(@Parameter(description = "Username substring") @QueryParam("filter") String filter) {
        ...
        List<UserDTO> userDTOs = users.stream()
                .map(user -> new UserDTO(user.getUsername(), user.getName(), user.getEmail(), user.getUserCreated(),
                        user.isAdmin(), user.getUserRole(), user.getId(), user.getManagedUsers()))
    ...
    @APIResponse(responseCode = "200", description = "The user information", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = PISUser.class)))
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ "admin", "employee" })
    public Response getUserSingle(
            @Parameter(description = "Username of the user") @PathParam("username") String username) {
        PISUser p = userMgr.findByUsername(username);
        if (p != null) {
            return Response.ok(p).build();
        } else {
            return Response.status(Status.NOT_FOUND).entity("User not found for username: " + username)
    ...
    @APIResponse(responseCode = "200", description = "The updated user information", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = PISUser.class)))
    @APIResponse(responseCode = "404", description = "User not found")
    @APIResponse(responseCode = "401", description = "Unauthorized")
    @Path("/{id}")
    @PUT
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ "admin", "employee" })
    public Response updateUserSingle(@Parameter(description = "ID of the user to update") @PathParam("id") Long id,
            PISUser src) {
    ...
    public Response patchUser(
            @Parameter(description = "Username of the user to update") @PathParam("username") String username,
            PISUser src) {
        PISUser p = userMgr.findByUsername(username);
        if (p != null) {
            ...
            if (src.getPassword() != null && !src.getPassword().isEmpty()
                    && PASSWORD_PATTERN.matcher(src.getPassword()).matches()) {
                p.setPassword(src.getPassword());
                isUpdated = true;
            }

            if (src.getEmail() != null && !src.getEmail().isEmpty()
                    && EMAIL_PATTERN.matcher(src.getEmail()).matches()) {
            ...
            if (isUpdated) {
                ...
                return Response.status(Status.BAD_REQUEST).entity("No valid updates provided")
                        .type(MediaType.APPLICATION_JSON).build();
            }
        } else {
            return Response.status(Status.NOT_FOUND).entity("User not found for username: " + username)
    ...
    public Response addUser(PISUser PISUser) {
        if (!USERNAME_PATTERN.matcher(PISUser.getUsername()).matches()) {
            return Response.status(Status.BAD_REQUEST).entity("Invalid username").type(MediaType.APPLICATION_JSON)
                    .build();
        }

        if (!PASSWORD_PATTERN.matcher(PISUser.getPassword()).matches()) {
            return Response.status(Status.BAD_REQUEST).entity("Invalid password").type(MediaType.APPLICATION_JSON)
                    .build();
        }
        ...
        if (existingByUsername != null) {
            return Response.status(Status.BAD_REQUEST).entity("Username is taken").type(MediaType.APPLICATION_JSON)
                    .build();
        }

        PISUser existingByEmail = userMgr.findByEmail(PISUser.getEmail());
        if (existingByEmail != null) {
            return Response.status(Status.BAD_REQUEST).entity("Email is taken").type(MediaType.APPLICATION_JSON)
                    .build();
        }
    ...
    public Response deleteUser(
            @Parameter(description = "Username of the user to delete") @PathParam("username") String username) {
        ...
        if (username.equals(o.getUsername())) {
            return Response.status(Status.BAD_REQUEST).entity("User cannot delete themselves")
                    .type(MediaType.APPLICATION_JSON).build();
        }
    ...
    public Response getManagedUsers(
            @Parameter(description = "Username of the user whose managed users to retrieve") @PathParam("username") String username) {
                ...
                .map(managedUsername -> {
                    ...
                    if (managedUser != null) {
                        return new UserDTO(managedUser.getUsername(), managedUser.getName(), managedUser.getEmail(),
                                managedUser.getUserCreated(), managedUser.isAdmin(), managedUser.getUserRole(),
                                managedUser.getId(), managedUser.getManagedUsers());
                    }
                    return null;
                })
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed({ "admin" })
    public Response addManagedUsers(
            @Parameter(description = "Username of the user to add managed users to") @PathParam("username") String username,
            AddManagedUsersRequest requestBody) {
        PISUser user = userMgr.findByUsername(username);
        if (user == null) {
            return Response.status(Status.NOT_FOUND).entity("User not found").type(MediaType.APPLICATION_JSON).build();
        }
        ...
        for (String managedUsername : managedUsernames) {
            PISUser managedUser = userMgr.findByUsername(managedUsername);
            if (managedUser == null) {
                return Response.status(Status.BAD_REQUEST).entity("Managed user not found: " + managedUsername)
                        .type(MediaType.APPLICATION_JSON).build();
            }
        }
